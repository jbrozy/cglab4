...
struct t_triangleIndex {
	...
	int ver[3];int tex[3]; int norm[3];
};
...
// load BMP from file "filename", pointer to "number" returns the number of vertices, returns VertexArray
myVertexType* loadModel(const char* filename, int* number)
{
	...
	fseek(f, 0, SEEK_SET);
	printf("Gefunden wurden %d Vertexe, %d Normalen, %d Texturekoordinaten und eine Tabelle mit %d Dreiecken\n", numVertices, numNormals, numTexture, numTable);
	...
	vertices = (myVertexType*)malloc(numTable * 3 * sizeof(myVertexType));//*3 because of three vertices per triangle
	...
	{
		vertices[i * 3] = myVertexType(vertexCoordinates[triangles[i].ver[0] - 1], vertexNormals[triangles[i].norm[0] - 1], vertexTextureUV[triangles[i].tex[0] - 1]);
		vertices[i * 3 + 1] = myVertexType(vertexCoordinates[triangles[i].ver[1] - 1], vertexNormals[triangles[i].norm[1] - 1], vertexTextureUV[triangles[i].tex[1] - 1]);
		vertices[i * 3 + 2] = myVertexType(vertexCoordinates[triangles[i].ver[2] - 1], vertexNormals[triangles[i].norm[2] - 1], vertexTextureUV[triangles[i].tex[2] - 1]);
